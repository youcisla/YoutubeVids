...
tmpdir = os.path.join(tempfile.gettempdir(), 'ah_subtitles')
...
for i in range(7):
    ...
    # Write ASS to temp dir with short name
    ass_tmp = os.path.join(tmpdir, f's{i}.ass')
    with open(ass_tmp, 'w', encoding='utf-8') as f:
        f.write(style_header + '\n'.join(ass_lines))
    
    # Write filter script to temp dir
    filter_script = os.path.join(tmpdir, f'vf{i}.txt')
    with open(filter_script, 'w') as f:
        f.write(f"ass={ass_tmp}")
    
    cmd = f'ffmpeg -y -i "{inp}" -filter_script:v "{filter_script}" -c:a copy "{out}"'
